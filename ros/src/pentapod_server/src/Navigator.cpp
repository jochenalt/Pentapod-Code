...
void Navigator::listenerGlobalPlan(const nav_msgs::Path::ConstPtr& og ) {
	...
	if (!navigationGoal.isNull() && latchedGoalOrientationToPath && (globalPlan.size() > 5)) {
		...
		// so identify the direction the bot is coming in by taking the vector of the last pieces
		const millimeter lastPieceLength = 400; // consider the radius of the bot as distance
		int curr = globalPlan.size()-1;
		StampedPose lastPose = globalPlan[curr];

		// go back in the global plan until we made 400mm
		while ((curr > 0) && (globalPlan[curr].pose.position.distance(lastPose.pose.position) < lastPieceLength))
			curr--;

		// compute the orientation of the goal and that point on the plan
		Point orientationVec =  globalPlan[curr].pose.position-navigationGoal_world.position;
		...
		Navigator::getInstance().setNavigationGoal(navigationGoal_world, false);
	}
}
