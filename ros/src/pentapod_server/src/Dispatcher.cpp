...
void Dispatcher::setup(ros::NodeHandle& handle) {
	...
	ros::Subscriber slamMapSubsrciber = handle.subscribe("map", 1000, &Dispatcher::listenerOccupancyGrid, this);

	// subscribe to the laser scaner directly in order to display the nice red pointcloud
	ROS_INFO_STREAM("subscribe to /scan");
	ros::Subscriber laserScanSubscriber = handle.subscribe("scan", 1000, &Dispatcher::listenToLaserScan, this);

	// subscribe to the SLAM topic that deliveres the etimated position
	ROS_INFO_STREAM("subscribe to /slam_out_pose");
	ros::Subscriber estimatedSLAMPoseSubscriber = handle.subscribe("slam_out_pose", 1000, &Dispatcher::listenerSLAMout,  this);

		// subscribe to the bots odom (without being fused with SLAM)
	ROS_INFO_STREAM("subscribe to /odom");
	ros::Subscriber odomSubscriber = handle.subscribe("odom", 1000, &Dispatcher::listenerOdometry, this);

	// subscribe to the bots state
	ROS_INFO_STREAM("subscribe to /engine/get_state");
	ros::Subscriber engineStateSubscriber = handle.subscribe("/engine/get_state", 1000, &Dispatcher::listenerBotState,  this);
...
// send map->odom transformation via tf
