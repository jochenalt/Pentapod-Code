...
*/
/**************************************************************************/
boolean Adafruit_VL6180X::begin(i2c_t3* newWireLine) {
...
uint8_t Adafruit_VL6180X::read8(uint16_t address)
{
  ...
  wireLine->beginTransmission(_i2caddr);
  wireLine->write(address>>8);
  wireLine->write(address);
  wireLine->endTransmission();

  wireLine->requestFrom(_i2caddr, (uint8_t)1);
  uint8_t r = wireLine->read();

#if defined(I2C_DEBUG)
  logger->print("\t$"); logger->print(address, HEX); logger->print(": 0x"); logger->println(r, HEX);
#endif
...
uint16_t Adafruit_VL6180X::read16(uint16_t address)
{
  ...
  wireLine->requestFrom(_i2caddr, (uint8_t)2);
  while(!wireLine->available());
  data = wireLine->read();
  data <<= 8;
  while(!wireLine->available());
  data |= wireLine->read();
...
#if defined(I2C_DEBUG)
  logger->print("\t$"); logger->print(address, HEX); logger->print(" = 0x"); logger->println(data, HEX);
#endif
}
