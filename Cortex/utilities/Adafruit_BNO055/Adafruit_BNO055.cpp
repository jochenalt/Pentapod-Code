...
*/
/**************************************************************************/
Adafruit_BNO055::Adafruit_BNO055(i2c_t3* newWireLine, int32_t sensorID, uint8_t address)
{
  _sensorID = sensorID;
  _address = address;
  wireLine = newWireLine;
}
...
#ifdef ESP8266
  wireLine->setClockStretchLimit(1000); // Allow for 1000us of clock stretching
#endif
...
imu::Vector<3> Adafruit_BNO055::getVector(adafruit_vector_type_t vector_type)
{
  ...
  bool ok = readLen((adafruit_bno055_reg_t)vector_type, buffer, 6);
  if (ok) {
...
*/
/**************************************************************************/
bool Adafruit_BNO055::getOrientationEvent(sensors_event_t *event)
{
  /* Clear the event */
...
bool Adafruit_BNO055::getAccelerationEvent(sensors_event_t *event)
{
  /* Clear the event */
  memset(event, 0, sizeof(sensors_event_t));
...
{
  wireLine->beginTransmission(_address);
  #if ARDUINO >= 100
    wireLine->write((uint8_t)reg);
    wireLine->write((uint8_t)value);
  #else
    wireLine->send(reg);
    wireLine->send(value);
  #endif
  wireLine->endTransmission();
...
byte Adafruit_BNO055::read8(adafruit_bno055_reg_t reg )
{
  ...
  #endif
  wireLine->endTransmission();
  wireLine->requestFrom(_address, (byte)1);
  #if ARDUINO >= 100
    value = wireLine->read();
  #else
    value = wireLine->receive();
  #endif
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, byte * buffer, uint8_t len)
{
  ...
  {
    #if ARDUINO >= 100
      buffer[i] = wireLine->read();
    #else
      buffer[i] = wireLine->receive();
    #endif
  }
